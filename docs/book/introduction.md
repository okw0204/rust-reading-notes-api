...
題材は、本、読書状態、メモを SQLite に保存する読書記録 API です。本の登録、一覧と絞り込み、詳細取得、状態変更、メモ追加、削除に加え、複数冊を読了時のメモとともに記録する一括読了記録を扱います。
...
図の往路では、HTTP の JSON が入力用の型へ変わり、検証済みの値を経て SQLite へ保存されます。復路では、DB の行を検証してドメインの型へ戻し、HTTP の出力用の型へ所有権を移して JSON にします。一括読了記録では、要求全体を先に検証してから、各本の読書状態とメモを一つの保存単位で確定します。`BookRepository` は別プロセスではなく、service と SQLite Adapter の間にある Interface です。

最初から図のすべてを理解する必要はありません。第 1 部では `String`、小さな変換関数、`Result` から始め、本の登録と一括読了記録の入力へ範囲を絞って読みます。

## 5 部の読み方

| 部 | 読む観点 | 読了時に説明したいこと |
| --- | --- | --- |
| 第 1 部：値と関数 | 所有権の移動、部分的な移動、借用、`Clone`、`Result` と `?` | HTTP の入力がどこへ移り、保存前に要求全体を検証する理由 |
| 第 2 部：型と状態 | 検証済み newtype、可視性、`enum`、型状態、外部データの検証 | 型が防ぐ不正と実行時の検査が必要な不正を分け、一冊の原子性と冊子間の部分成功を説明できること |
