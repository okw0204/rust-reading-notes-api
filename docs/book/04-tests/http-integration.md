...
全体を振り返ると、型状態の doctest は書ける操作、フェイクは service の判断、SQLite テストは保存の契約、Router テストは HTTP を含む入出力を確かめています。[教材の使い方](../introduction.md#4部の読み方)や画面の目次から気になる章へ戻り、「どの値を、どの境界で、何を根拠に保証するか」を説明してみてください。
